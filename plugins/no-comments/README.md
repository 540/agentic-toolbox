...
- Vigila también `Bash`, pero solo para redirigir: si un comando reescribe un fichero de código (`>`, `>>`, `tee`, `sed -i`, `perl -i`, un heredoc de Python que llama a `write_text` o `open`), lo deniega pidiendo que use `Edit` o `Write`. No mira el contenido del comando ni entiende shell; solo empuja al agente al camino donde el hook sí mira. Un agente bloqueado tiende a buscar la salida por la shell, y por ahí un comentario entra sin que nadie lo apruebe.
